...
###################################################
{
  ...
  setwd("C:/Users/Tshih/OneDrive/Distance between dental clinics and metro and bus stations/Data")
...

{
  ...
  names_dist <- read_excel("2025_06_22_List_of_Riyadh_dsitricts.xlsx", sheet = "Clean") %>% 
    clean_names() %>% 
...

{
  ...
  pbf_file      <- "C:/Users/Tshih/OneDrive/Distance between dental clinics and metro and bus stations/Data/gcc-states-251211.osm.pbf"
  out_rds       <- "C:/Users/Tshih/OneDrive/Distance between dental clinics and metro and bus stations/Data/graph_c_motorcar_clipped.rds"
...
out_rds_c <- "C:/Users/Tshih/OneDrive/Distance between dental clinics and metro and bus stations/Data/graph_c_motorcar_clipped_OPTION1_directed.rds"
out_rds_w <- "C:/Users/Tshih/OneDrive/Distance between dental clinics and metro and bus stations/Data/graph_w_motorcar_clipped_OPTION1_directed.rds"
meta_rds <- "C:/Users/Tshih/OneDrive/Distance between dental clinics and metro and bus stations/Data/clinics_geo_vs_network_4strata_with_meta.rds"
...
library(dodgr)
  ...
  graph_c <- readRDS("C:/Users/Tshih/OneDrive/Distance between dental clinics and metro and bus stations/Data/graph_c_motorcar_clipped.rds")
  ...
  # ---- Save for reuse ----
  roads_out <- "C:/Users/Tshih/OneDrive/Distance between dental clinics and metro and bus stations/Data/roads_sf_riyadh_clipped_32638.rds"
  saveRDS(roads_sf, roads_out)
  
  cat("Saved roads_sf to:\n", roads_out, "\n")
  
  # load road_sf
  roads_sf <- readRDS("C:/Users/Tshih/OneDrive/Distance between dental clinics and metro and bus stations/Data/roads_sf_riyadh_clipped_32638.rds")
  ...
  
{
  ...
  # ---- Parameters ----
  target_crs    <- 32638
  pbf_graph_rds <- "C:/Users/Tshih/OneDrive/Distance between dental clinics and metro and bus stations/Data/graph_c_motorcar_clipped_OPTION1_directed.rds"
  out_dir       <- "C:/Users/Tshih/OneDrive/Distance between dental clinics and metro and bus stations/Data"
...
# 1. FILE PATHS
# --------------------------------------------------------------------------
base_dir <- "C:/Users/Tshih/OneDrive/Distance between dental clinics and metro and bus stations/Data"
...
stats_rnd <- dat_rnd %>%
  ...
  mutate(
    ...
    to_ord  = match(Point2, c("Nearest bus stop", "Nearest metro station", "Nearest private dental facility", "Median-distance private dental facility", "Nearest public dental facility", "Median-distance public dental facility"))
  )
...
robust_rds <- "C:/Users/Tshih/OneDrive/Distance between dental clinics and metro and bus stations/Data/clinics_geo_vs_network_ROBUST_DIRECTED.rds"
...
# File paths
rp_rds <- "C:/Users/Tshih/OneDrive/Distance between dental clinics and metro and bus stations/Data/random_points_geo_vs_network_EDGE_METHOD_populated_only.rds"
fac_rds <- "C:/Users/Tshih/OneDrive/Distance between dental clinics and metro and bus stations/Data/clinics_geo_vs_network_ROBUST_DIRECTED.rds"
roads_fp <- "C:/Users/Tshih/OneDrive/Distance between dental clinics and metro and bus stations/Data/roads_sf_riyadh_clipped_32638.rds"
...
get_L1_from_lookup <- function(rp_id, transit_type, L1_metro_lookup, L1_bus_lookup, ratio_lookup) {
  if(transit_type == "metro") {
    row <- L1_metro_lookup %>% filter(id == rp_id)
    if(nrow(row) == 0) return(NA_real_)
    if(!is.na(row$L1_metro_net_km) && !is.infinite(row$L1_metro_net_km)) return(row$L1_metro_net_km * 1000)
    ratio <- ratio_lookup["nearest_metro"]; if(is.na(ratio)) ratio <- 1.3
    return(row$L1_metro_geo_km * ratio * 1000)
  } else {
    row <- L1_bus_lookup %>% filter(id == rp_id)
    if(nrow(row) == 0) return(NA_real_)
    if(!is.na(row$L1_bus_net_km) && !is.infinite(row$L1_bus_net_km)) return(row$L1_bus_net_km * 1000)
    ratio <- ratio_lookup["nearest_bus"]; if(is.na(ratio)) ratio <- 1.3
    return(row$L1_bus_geo_km * ratio * 1000)
  }
}
...
get_L3_from_lookup <- function(rp_id, dest_type, transit_type, clinic_targets, L3_metro_lookup, L3_bus_lookup, ratio_L3_metro_priv, ratio_L3_metro_pub, ratio_L3_bus_priv, ratio_L3_bus_pub) {
  target_row <- clinic_targets %>% filter(id == rp_id, dest_type == !!dest_type)
  if(nrow(target_row) == 0) return(NA_real_)
  clinic_id <- target_row$clinic_id[1]
  is_private <- grepl("priv", dest_type)
  if(transit_type == "metro") {
    row <- L3_metro_lookup %>% filter(clinic_id == !!clinic_id)
    if(nrow(row) == 0) return(NA_real_)
    if(!is.na(row$L3_metro_net_km[1]) && !is.infinite(row$L3_metro_net_km[1])) return(row$L3_metro_net_km[1] * 1000)
    ratio <- if(is_private) ratio_L3_metro_priv else ratio_L3_metro_pub
    return(row$L3_metro_geo_km[1] * ratio * 1000)
  } else {
    row <- L3_bus_lookup %>% filter(clinic_id == !!clinic_id)
    if(nrow(row) == 0) return(NA_real_)
    if(!is.na(row$L3_bus_net_km[1]) && !is.infinite(row$L3_bus_net_km[1])) return(row$L3_bus_net_km[1] * 1000)
    ratio <- if(is_private) ratio_L3_bus_priv else ratio_L3_bus_pub
    return(row$L3_bus_geo_km[1] * ratio * 1000)
  }
}
...
for(i in 1:nrow(rp_sample)) {
  ...
  near_metro_rp <- find_nearest_transit(rp_pt, stations_proj, "metro")
  near_bus_rp   <- find_nearest_transit(rp_pt, bus_stops_proj, "bus")
  near_bus_rt_type <- bus_stops_proj$route_type[near_bus_rp$idx]
  if(is.na(near_bus_rt_type)) near_bus_rt_type <- "Standard"
  ...
  for(dest_type in dest_types) {
    ...
    clinic_id <- target_row$clinic_id[1]
    clinic_pt <- if(grepl("^priv_", target_row$dest_id_geo[1])) {
      clinics_private %>% filter(id == clinic_id)
    } else {
      clinics_public %>% filter(id == clinic_id)
    }
    if(nrow(clinic_pt) == 0) next
    ...
    near_metro_cl <- find_nearest_transit(clinic_pt, stations_proj, "metro")
    near_bus_cl   <- find_nearest_transit(clinic_pt, bus_stops_proj, "bus")
    near_bus_cl_rt_type <- bus_stops_proj$route_type[near_bus_cl$idx]
    if(is.na(near_bus_cl_rt_type)) near_bus_cl_rt_type <- "Standard"
    
    # Comparison Baseline
    comp_row <- rp_comparison %>% filter(id == rp_id, dest_type == !!dest_type)
    road_dist_m <- NA_real_
    if(nrow(comp_row) > 0) {
      val_km <- comp_row$net_km_p2t
      imp_factor <- 1.3
      ...
      if(is.na(val_km) || is.infinite(val_km)) val_km <- comp_row$geo_km * imp_factor
...
ggsave("Fig_mean_travel_time.tiff", p_mean, width = 14, height = 9, dpi = 600)
...
if (car_label %in% names(df_wide)) {
  ...
  ggsave("Fig_percent_faster_integrated.tiff", p_pct, width = 14, height = 9, dpi = 600)
}
...
if (exists("df_cmp")) {
  ...
  ggsave("Fig_mean_time_savings.tiff", p_savings_trend, width = 14, height = 9, dpi = 600)
}
...
rp_rds <- "C:/Users/Tshih/OneDrive/Distance between dental clinics and metro and bus stations/Data/random_points_geo_vs_network_EDGE_METHOD_pop_weighted_NO_LCC.rds"
fac_rds <- "C:/Users/Tshih/OneDrive/Distance between dental clinics and metro and bus stations/Data/clinics_geo_vs_network_ROBUST_DIRECTED.rds"
roads_fp <- "C:/Users/Tshih/OneDrive/Distance between dental clinics and metro and bus stations/Data/roads_sf_riyadh_clipped_32638.rds"
...
for(i in 1:nrow(rp_sample)) {
  ...
  rp_id <- rp_sample$id[i]; rp_pt <- rp_sample[i, ]
  ...
  for(dest_type in dest_types) {
    ...
    if(nrow(target_row) == 0) next
    clinic_id <- target_row$clinic_id[1]
    clinic_pt <- if(grepl("^priv_", target_row$dest_id_geo[1])) clinics_private %>% filter(id == clinic_id) else clinics_public %>% filter(id == clinic_id)
    if(nrow(clinic_pt) == 0) next
...
if (exists("df_cmp")) {
  ...
  View(print(as.data.frame(diagnostic_stats)))
...
candidates <- results_df %>% filter(!is.na(multi_total_m))
